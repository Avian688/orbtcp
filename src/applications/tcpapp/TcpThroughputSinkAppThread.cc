...
long TcpThroughputSinkAppThread::computeThroughput(bool peerClosed) {
    long thr;
    if (!peerClosed) {
        ...
        thr = (bytesRcvd - lastBytesReceived) * 8 / (simTime().dbl() - lastThroughputTime.dbl());
        ...
        thr = (bytesRcvd - oldLastBytesReceived) * 8 / (simTime().dbl() - oldlastThroughputTime.dbl());
        EV_TRACE << "Throughput computed from application: " << thr << std::endl;
    }
    return thr;
}
...
void TcpThroughputSinkAppThread::peerClosed() {
    ...
    long thr = computeThroughput(true);
    lastBytesReceived = bytesRcvd;
    lastThroughputTime = simTime();
