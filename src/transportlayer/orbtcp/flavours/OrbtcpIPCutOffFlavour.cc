...
#include "OrbtcpIPCutOffFlavour.h"
...
Register_Class(OrbtcpIPCutOffFlavour);

simsignal_t OrbtcpIPCutOffFlavour::txRateSignal = cComponent::registerSignal("txRate");
simsignal_t OrbtcpIPCutOffFlavour::tauSignal = cComponent::registerSignal("tau");
simsignal_t OrbtcpIPCutOffFlavour::uSignal = cComponent::registerSignal("u");
simsignal_t OrbtcpIPCutOffFlavour::USignal = cComponent::registerSignal("U");
simsignal_t OrbtcpIPCutOffFlavour::additiveIncreaseSignal = cComponent::registerSignal("additiveIncrease");
simsignal_t OrbtcpIPCutOffFlavour::sharingFlowsSignal = cComponent::registerSignal("sharingFlows");
simsignal_t OrbtcpIPCutOffFlavour::bottleneckBandwidthSignal = cComponent::registerSignal("bottleneckBandwidth");
simsignal_t OrbtcpIPCutOffFlavour::avgRttSignal = cComponent::registerSignal("avgRtt");
simsignal_t OrbtcpIPCutOffFlavour::queueingDelaySignal = cComponent::registerSignal("queueingDelay");
simsignal_t OrbtcpIPCutOffFlavour::estimatedRttSignal = cComponent::registerSignal("estimatedRtt");
simsignal_t OrbtcpIPCutOffFlavour::avgEstimatedRttSignal = cComponent::registerSignal("avgEstimatedRtt");
simsignal_t OrbtcpIPCutOffFlavour::alphaSignal = cComponent::registerSignal("alpha");

OrbtcpIPCutOffFlavour::OrbtcpIPCutOffFlavour() : OrbtcpFamily(),
    state((OrbtcpIPCutOffStateVariables *&)TcpAlgorithm::state)
{
}

void OrbtcpIPCutOffFlavour::initialize()
{
...
void OrbtcpIPCutOffFlavour::established(bool active)
{
...
void OrbtcpIPCutOffFlavour::rttMeasurementComplete(simtime_t tSent, simtime_t tAcked)
{
    //
...
double OrbtcpIPCutOffFlavour::measureInflight(IntDataVec intData)
{
    double u = 0;
    double tau;
    ...
    if(!state->initialPhase || state->snd_cwnd > state->ssthresh){ //slow start - more aggressive till max allowed share is reached
        ...
        if((state->snd_cwnd + state->additiveIncrease) > ((bottleneckBandwidth * rtt.dbl())*state->eta)/state->sharingFlows){
            state->additiveIncrease = (((bottleneckBandwidth * rtt.dbl())*state->eta)/state->sharingFlows) - state->snd_cwnd;
            state->initialPhase = false;
        }
    }
...
uint32_t OrbtcpIPCutOffFlavour::computeWnd(double u, bool updateWc)
{
    uint32_t w;
...
size_t OrbtcpIPCutOffFlavour::getConnId()
{
    return connId;
}
