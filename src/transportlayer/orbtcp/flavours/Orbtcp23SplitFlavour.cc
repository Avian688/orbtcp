...
#include "Orbtcp23SplitFlavour.h"
...
Register_Class(Orbtcp23SplitFlavour);

simsignal_t Orbtcp23SplitFlavour::txRateSignal = cComponent::registerSignal("txRate");
simsignal_t Orbtcp23SplitFlavour::tauSignal = cComponent::registerSignal("tau");
simsignal_t Orbtcp23SplitFlavour::uSignal = cComponent::registerSignal("u");
simsignal_t Orbtcp23SplitFlavour::USignal = cComponent::registerSignal("U");
simsignal_t Orbtcp23SplitFlavour::additiveIncreaseSignal = cComponent::registerSignal("additiveIncrease");
simsignal_t Orbtcp23SplitFlavour::sharingFlowsSignal = cComponent::registerSignal("sharingFlows");
simsignal_t Orbtcp23SplitFlavour::bottleneckBandwidthSignal = cComponent::registerSignal("bottleneckBandwidth");
simsignal_t Orbtcp23SplitFlavour::avgRttSignal = cComponent::registerSignal("avgRtt");
simsignal_t Orbtcp23SplitFlavour::queueingDelaySignal = cComponent::registerSignal("queueingDelay");
simsignal_t Orbtcp23SplitFlavour::estimatedRttSignal = cComponent::registerSignal("estimatedRtt");
simsignal_t Orbtcp23SplitFlavour::avgEstimatedRttSignal = cComponent::registerSignal("avgEstimatedRtt");
simsignal_t Orbtcp23SplitFlavour::alphaSignal = cComponent::registerSignal("alpha");

Orbtcp23SplitFlavour::Orbtcp23SplitFlavour() : OrbtcpFamily(),
    state((Orbtcp23SplitStateVariables *&)TcpAlgorithm::state)
{
}

void Orbtcp23SplitFlavour::initialize()
{
...
void Orbtcp23SplitFlavour::established(bool active)
{
...
void Orbtcp23SplitFlavour::rttMeasurementComplete(simtime_t tSent, simtime_t tAcked)
{
    //
...
double Orbtcp23SplitFlavour::measureInflight(IntDataVec intData)
{
    double u = 0;
    double tau;
    ...
    if(state->ssComplete || state->snd_cwnd > state->ssthresh){ //slow start - more aggressive till max allowed share is reached
        ...
        state->additiveIncrease = ((bottleneckBandwidth * rtt.dbl())*(state->additiveIncreasePercent*0.4))/state->sharingFlows;
        state->ssthresh = 0;
        ...
        state->additiveIncrease = (bottleneckBandwidth * rtt.dbl())*(state->additiveIncreasePercent*0.6);
    }
...
size_t Orbtcp23SplitFlavour::getConnId()
{
    return connId;
}
...
unsigned int Orbtcp23SplitFlavour::getCwnd()
{
