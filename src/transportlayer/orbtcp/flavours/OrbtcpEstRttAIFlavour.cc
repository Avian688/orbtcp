...
#include "OrbtcpEstRttAIFlavour.h"
...
Register_Class(OrbtcpEstRttAIFlavour);

simsignal_t OrbtcpEstRttAIFlavour::txRateSignal = cComponent::registerSignal("txRate");
simsignal_t OrbtcpEstRttAIFlavour::tauSignal = cComponent::registerSignal("tau");
simsignal_t OrbtcpEstRttAIFlavour::uSignal = cComponent::registerSignal("u");
simsignal_t OrbtcpEstRttAIFlavour::USignal = cComponent::registerSignal("U");
simsignal_t OrbtcpEstRttAIFlavour::additiveIncreaseSignal = cComponent::registerSignal("additiveIncrease");
simsignal_t OrbtcpEstRttAIFlavour::sharingFlowsSignal = cComponent::registerSignal("sharingFlows");
simsignal_t OrbtcpEstRttAIFlavour::bottleneckBandwidthSignal = cComponent::registerSignal("bottleneckBandwidth");
simsignal_t OrbtcpEstRttAIFlavour::avgRttSignal = cComponent::registerSignal("avgRtt");
simsignal_t OrbtcpEstRttAIFlavour::queueingDelaySignal = cComponent::registerSignal("queueingDelay");
simsignal_t OrbtcpEstRttAIFlavour::estimatedRttSignal = cComponent::registerSignal("estimatedRtt");
simsignal_t OrbtcpEstRttAIFlavour::avgEstimatedRttSignal = cComponent::registerSignal("avgEstimatedRtt");
simsignal_t OrbtcpEstRttAIFlavour::alphaSignal = cComponent::registerSignal("alpha");

OrbtcpEstRttAIFlavour::OrbtcpEstRttAIFlavour() : OrbtcpFamily(),
    state((OrbtcpEstRttAIStateVariables *&)TcpAlgorithm::state)
{
}

void OrbtcpEstRttAIFlavour::initialize()
{
...
void OrbtcpEstRttAIFlavour::established(bool active)
{
...
void OrbtcpEstRttAIFlavour::rttMeasurementComplete(simtime_t tSent, simtime_t tAcked)
{
    //
...
void OrbtcpEstRttAIFlavour::receivedDataAckInt(uint32_t firstSeqAcked, IntDataVec intData)
{
...
double OrbtcpEstRttAIFlavour::measureInflight(IntDataVec intData)
{
    double u = 0;
    double tau;
    ...
    if(!state->initialPhase || state->snd_cwnd > state->ssthresh){ //slow start - more aggressive till max allowed share is reached
        ...
        state->additiveIncrease = ((bottleneckBandwidth * estimatedRtt.dbl())*(state->additiveIncreasePercent));
    }
...
uint32_t OrbtcpEstRttAIFlavour::computeWnd(double u, bool updateWc)
{
    uint32_t w;
...
size_t OrbtcpEstRttAIFlavour::getConnId()
{
    return connId;
}

simtime_t OrbtcpEstRttAIFlavour::getSrtt()
{
    //return state->srtt;
    return estimatedRtt;
}

unsigned int OrbtcpEstRttAIFlavour::getCwnd()
{
